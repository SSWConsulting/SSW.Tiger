...
async function testValidation() {
  ...
  // Graph API sends POST with validationToken as query param during subscription creation
  const response = await fetch(
    `${FUNCTION_URL}?validationToken=test-token-12345`,
    {
      method: "POST",
    },
  );
...
async function testMockNotification() {
  ...
  console.log(
    `Result: ${result.results?.[0]?.success ? "✅ PASS" : "❌ FAIL"}`,
  );
}
...
async function testNonSprintMeeting() {
  ...
  console.log(`Response:`, JSON.stringify(result, null, 2));
  console.log(
    `Result: ${result.results?.[0]?.skipped ? "✅ PASS (skipped as expected)" : "❌ FAIL"}`,
  );
}
...
async function testProjectNameExtraction() {
  ...
  const testCases = [
    ...
    {
      input: "This is a long meeting name without any prefix",
      expected: "general",
    },
...
async function testFilenameGeneration() {
  ...
  const testCases = [
    // Bracket format
    {
      input: {
        subject: "[YakShaver] Sprint Review",
        startDateTime: "2026-01-23T10:00:00Z",
      },
    ...
    },
    {
      input: {
        subject: "[Tiger] Daily Standup",
        startDateTime: "2026-01-26T09:00:00Z",
      },
  ...
  for (const { input, expected } of testCases) {
    ...
    const status = result === expected ? "✅" : "❌";
    console.log(
      `  ${status} "${input.subject}" → "${result}" (expected: "${expected}")`,
    );
    if (result === expected) passed++;
  }
