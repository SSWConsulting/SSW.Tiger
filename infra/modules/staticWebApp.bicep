...
// SWA-proxied requests are accepted. VERIFY it exists after deploy (see the
// post-deploy note in main.bicep / README) — it is what makes the backend's
...
@secure()
@description('Entra (AAD) app registration client ID, reused from the Graph app for user login. Marked secure only so it can be sourced from Key Vault via getSecret().')
param entraClientId string

@secure()
@description('Client secret for the reused Entra app registration.')
