...
async function upsertMeetingSecurity({
  ...
  const missing = [
    !projectName && "projectName",
    !meetingId && "meetingId",
    passwordEnabled && !passwordSecretName && "passwordSecretName",
  ].filter(Boolean);
  ...
  const document = {
    ...
    passwordSecretName: passwordSecretName || null,
...
async function listProjects() {
  ...
  const { resources } = await container.items
    .query("SELECT DISTINCT VALUE c.projectName FROM c WHERE NOT IS_DEFINED(c.type) OR c.type != 'meetingSecurity'")
    .fetchAll();
