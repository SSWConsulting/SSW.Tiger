 ...
 * @param {string} [params.failureReason] - user-facing reason, set on failure. Cleared
 *                                          on any non-failed status so a retry that
 *                                          succeeds does not keep showing the old error.
 ...
 * @param {boolean} [params.passwordProtected] - whether the dashboard is password-protected. Pass
 *                                          false to clear it; omit (undefined) to leave it untouched.
 * @param {string|null} [params.dashboardPassword] - the dashboard password, so the portal can show it to
 *                                          the owner (portal submissions get no Teams notification).
 *                                          Owner-scoped: only the submitter can read their own record.
 *                                          Pass null/"" to clear it; omit to leave it untouched.
 * @returns {Object|null} the patched record, or null if no record exists
 */
function buildSubmissionPatchOperations({
  status,
  dashboardUrl,
  failureReason,
  displayName,
  ...
  // "set" unconditionally rather than only when present: a record moving OUT of
  // "failed" (a re-run that gets further) must not keep displaying the stale
  // reason. "set" also creates the path when it doesn't exist, unlike "replace".
  operations.push({ op: "set", path: "/failureReason", value: failureReason || null });
  if (displayName) {
    operations.push({ op: "set", path: "/displayName", value: displayName });
  }
  // Same reasoning as failureReason, and it matters more here: these carry a SECRET.
  // A re-run that produces an unprotected dashboard, or rotates the password, must be
  // able to CLEAR them — patching only on a truthy value would leave the old password
  // readable in the portal history forever. `undefined` still means "caller has
  // nothing to say"; false/null/"" are written through as a clear.
  if (passwordProtected !== undefined) {
    operations.push({ op: "set", path: "/passwordProtected", value: !!passwordProtected });
  }
  if (dashboardPassword !== undefined) {
    operations.push({ op: "set", path: "/dashboardPassword", value: dashboardPassword || null });
  }
  return operations;
}

async function updateSubmissionStatus({ requestId, projectName, ...fields }) {
  const container = getSubmissionsContainer();
  const operations = buildSubmissionPatchOperations(fields);
  try {
...
module.exports = {
  ...
  buildSubmissionPatchOperations,
  upsertMeeting,
  queryMeetings,
  getMeeting,
  listProjects,
