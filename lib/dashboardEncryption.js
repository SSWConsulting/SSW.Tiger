...
function renderUnlockPage({ projectName, meetingId, meetingTitle, encryptedPayload }) {
  const payloadJson = JSON.stringify(encryptedPayload);
  const escapedProject = escapeHtml(projectName);
  const escapedMeetingTitle = escapeHtml(meetingTitle);
  const meetingMeta = escapedMeetingTitle
    ? `    <div class="meta">${escapedMeetingTitle}</div>\n`
    : "";
