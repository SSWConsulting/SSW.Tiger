...
function createDashboardSecurityManager(deps = {}) {
  ...
  async function protectMeeting(target) {
    ...
    if (services.extractEncryptedPayloadFromUnlockPage(originalHtml)) {
      throw new Error("Dashboard HTML is already a Tiger encrypted unlock page; use rotate instead");
    }

    const password = services.generateDashboardPassword();
    const encryptedPayload = services.encryptDashboardHtml(originalHtml, password);
    const unlockHtml = services.renderUnlockPage({
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Failed to protect meeting dashboard and rollback dashboard state: ${error.message}; rollback: ${rollbackError.message}`,
        );
      }
      throw error;
    }
