...
describe("dashboard security manager", () => {
  ...
  it("protects an existing public dashboard without checking project policy", async () => {
    let uploadedHtml = null;
    let upserted = null;
    const publicDashboardHtml = "<html><body>Public dashboard</body></html>";
    const manager = createManager({
      downloadDashboardHtml: async () => publicDashboardHtml,
      uploadDashboardHtml: async (_project, _meeting, html) => {
        uploadedHtml = html;
      },
      upsertMeetingSecurity: async (document) => {
    ...
    assert.equal(uploadedHtml.includes("Sprint review"), true);
    assert.equal(upserted.projectName, "crm");
    assert.equal(upserted.meetingId, "2026-07-10-001");
    assert.equal(upserted.passwordEnabled, true);
    assert.equal(upserted.updatedBy, "Willow");
    assert.equal(
      decryptDashboardPassword(upserted.passwordEncryption, NEW_KEY),
      "NEWP-ASSW-ORD2",
    );
  ...
  it("rejects protect when the dashboard is already password protected", async () => {
    const manager = createManager({
      downloadDashboardHtml: async () =>
    ...
    await assert.rejects(
      () =>
        manager.protectMeeting({
          projectName: "crm",
          meetingId: "2026-07-10-001",
        }),
      /already a Tiger encrypted unlock page/,
    );
  });

  it("rolls back public dashboard HTML when protect cannot write Cosmos", async () => {
    const uploads = [];
    const publicDashboardHtml = "<html><body>Public dashboard</body></html>";
    const manager = createManager({
      downloadDashboardHtml: async () => publicDashboardHtml,
    ...
    assert.notEqual(uploads[0], publicDashboardHtml);
    assert.equal(uploads[1], publicDashboardHtml);
  });
